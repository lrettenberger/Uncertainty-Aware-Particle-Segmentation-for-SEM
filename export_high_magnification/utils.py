...
def write_parameters_to_file(inst_segmentations,file_name,image_name,image,resize_factor):
    # Calculate the sizes of the detected particles as pixel sizes
    # Writing it to a report file
    sizes_by_pixels = np.array(get_particle_sizes(inst_segmentations))*resize_factor

    # Write small report about the sizes
    f = open(file_name, "w")
    f.write(f'Report for {image_name} \n')
    f.write('----------------------------------- \n')
    image_size = np.prod(image.shape)*resize_factor
