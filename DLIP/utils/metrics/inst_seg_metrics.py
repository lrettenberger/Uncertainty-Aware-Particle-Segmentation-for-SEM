...
#extension -> make metrics numerically stable via eps in denom                                 #
################################################################################################
...
# --------------------------Optimised for Speed
def get_fast_aji(true, pred, eps=1e-6):
    ...
    pairwise_iou = pairwise_inter / (pairwise_union + eps)
...
#####
def get_fast_aji_plus(true, pred, eps=1e-6):
    ...
    for pred_id in unpaired_pred:
        overall_union += pred_masks[pred_id].sum()
    #
    aji_score = (overall_inter + np.finfo(np.float32).eps) / (overall_union + np.finfo(np.float32).eps)
    return aji_score


#####
def get_fast_pq(true, pred, match_iou=0.5, eps=1e-6):
    ...
    sq = paired_iou.sum() / (tp + eps)

    return [dq, sq, dq * sq], [paired_true, paired_pred, unpaired_true, unpaired_pred]


#####
def get_fast_dice_2(true, pred,eps=1e-6):
    """Ensemble dice."""
    true = np.copy(true)
    pred = np.copy(pred)
    ...
    return 2 * overall_inter / (overall_total+eps)


#####--------------------------As pseudocode
def get_dice_1(true, pred, eps=1e-6):
    """Traditional dice."""
    # cast to binary 1st
    true = np.copy(true)
    pred = np.copy(pred)
    ...
    inter = true * pred
    denom = true + pred
    return (2.0 * np.sum(inter)) / (np.sum(denom)+eps)


####
def get_dice_2(true, pred, eps=1e-6):
    ...
    return (2 * total_intersect) / (total_markup+eps)
